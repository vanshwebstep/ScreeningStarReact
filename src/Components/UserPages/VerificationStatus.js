...
const VerificationStatus = () => {
    ...
    const [bulkDownloadmap, setBulkDownloadmap] = useState([]);
    ...
    const [pdfExist, setPdfExist] = useState(null);
    ...
    const fetchData = useCallback((filterStatus = null) => {
        // console.log('fiwpfsji', filterStatus)
        if (!branch_id || !token) {
            return;
        } else {
        ...
        if (branchData?.type === "sub_user" && branchData.id) {
            baseUrl += `&sub_user_id=${branchData.id}`;
        }
        ...
        fetch(finalUrl, requestOptions)
            ...
            .then((result) => {
                ...
                setCompanyName(result.data.customerInfo.name);
                setFilterData(result?.data.filterOptions || []);

                const visible = result?.data?.customerInfo?.visible_fields
                setVisibleFields(JSON.parse(visible))
                ...
                const emailArray = JSON.parse(result?.data?.customerInfo?.emails);
    ...
    // console.log('loadinggggggggg1 -', viewLoading);
    // console.log('pdfExist1-', pdfExist);
    // console.log('ActiveService1-', activeService);
    const fetchServicesData = async (applicationId, servicesList, reportDownload = '0') => {
        ...
        try {
            ...
            if (result?.status === false && result?.message?.includes("internal error occurred while validating the token")) {
                await Swal.fire({
                    icon: 'error',
                    title: 'Validation Error',
                    text: result.message || 'An error occurred while validating the token.',
                });
                setViewLoading(null);
                setActiveService(null);
                return;
    ...
    //     navigate(`/admin-generate-report?applicationId=${applicationId}&branchid=${}`);
    // };
    ...
    const handleExportToExcel = () => {
        const dataToExport = selectedRows.length > 0 ?
            filteredData.filter((data) => selectedRows.includes(data.id)) :
            filteredData;
    ...
    return (
        <div className="bg-[#c1dff2]">
            <div className="space-y-4 py-[30px] border border-black md:px-[51px] px-6 bg-white">
                        ...
                        <table ref={scrollContainerRef} className="min-w-full border-collapse border border-black overflow-scroll rounded-lg whitespace-nowrap">
                            <thead className='rounded-lg'>
                                <tr className="bg-[#c1dff2] text-[#4d606b]">
                                    ...
                                    {(Array.isArray(visibleFields) ? visibleFields : []).map((field) => (
                                        <th key={field} className="uppercase border border-black px-4 py-2">
                                    ...
                                    <th className="text-left p-2 border border-black uppercase">First Level Insuff</th>
                                    <th className="text-left p-2 border border-black uppercase">First Level Insuff Date</th>
                                    <th className="text-left p-2 border border-black uppercase">First Level Insuff Reopen Date</th>
                                    <th className="text-left p-2 border border-black uppercase">Second Level Insuff</th>
                                    <th className="text-left p-2 border border-black uppercase">Second Level Insuff Date</th>
                                    <th className="text-left p-2 border border-black uppercase">Third Level Insuff Marks</th>
                                    <th className="text-left p-2 border border-black uppercase">Third Level Insuff Date</th>
                                    <th className="text-left p-2 border border-black uppercase">Third Level Insuff Reopen Date</th>
                                    <th className="text-left p-2 border border-black uppercase">Reason For Delay</th>
                                            ...
                                            return (
                                                <React.Fragment key={data.id}>
                                                    <tr className="text-center">
                                                        ...
                                                        {/* <td className="border border-black px-4 py-2">{data.tat_days || 'NIL'}</td> */}
                                                        <td className="border border-black px-4 py-2">{data.name || 'NIL'}</td>
                                                        {visibleFields.map((field) => (
                                                            <td key={field} className="border border-black px-4 py-2 uppercase">
                                                                {field === 'photo' ? (
                                                                    <div className='w-full text-center flex justify-center'>
                                                                        <img src={data.photo ? data.photo : Default} alt={data.name} className="w-10 h-10 rounded-full" />
                                                                    </div>
                                                                ) : field === 'gender' ? (
                                                                    data.gender || 'N/A'
                                                                ) : field === 'location' ? (
                                                        ...
                                                        <td className="border border-black px-4 py-2">{data.application_id}</td>
                                                        <td className="border border-black px-4 py-2">{formatDate(data.initiation_date)}</td>
